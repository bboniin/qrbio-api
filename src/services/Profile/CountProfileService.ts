...
class CountProfileService {
  async execute({ profile_id, ip }: CountRequest) {
    ...
    if (
      plans[profile.plan_name] * 0.8 == countProfile.length + 1 &&
      profile.user.email_confirmation &&
      emailEnviado.length == 0
    ) {
      const path = resolve(__dirname, "..", "..", "views", "viewsProfile.hbs");
    ...
    if (
      plans[profile.plan_name] == countProfile.length + 1 &&
      profile.user.email_confirmation &&
      emailEnviado.length == 0
    ) {
      const path = resolve(
